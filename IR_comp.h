...
struct IR_comp:IR_front{
    ...
    void compress_number(int num,std::ofstream& save_to_disk,int &tp_index);
    ...
    void posting_compression(){
        ...
        for(int i=0; i<num_terms;i++) {
            ...
            compress_number(v[0],save_pos_toDisk,tp_index);
            
            for (int j=1;j<v.size();j++)  compress_number(v[j]-v[j-1],save_pos_toDisk,tp_index);
...
void IR_comp::compress_number(int num,std::ofstream& save_to_disk, int &tp_index){
    if(num<128){
